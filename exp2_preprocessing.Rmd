...
This script run the data pre-processing of experiment 2.

Dataset loaded: **exp_soa_data.csv** (data of *result verification task*)
...
# Load data (included )
exp2_rowData <- read_csv(here('data', 'rawdata', 'exp_soa_data.csv'))
```
...
* **SJ 15** has low accuracy (about 50%) in block 2.
* **SJ 17** has low accuracy (about 50%) in block 10.
* **SJ 24** has low accuracy (about 50%) in block 6.
* **SJ 28** has low accuracy (about 26%) in block 1.
* **SJ 29** has low accuracy (about 56%) in block 8.
* **SJ 31** has low accuracy (about 62%) in block 10.
...
The processed data are saved in the file **data/processed/exp2_data.rds**.

```{r}
saveRDS(
  object = exp2_rt_outlier,
  file = here('data', 'processed', 'exp2_data.rds'))
```
